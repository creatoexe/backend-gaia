  ...
  try {
              ...
              include: [
                {
                  ...
                  as: "propuesta",
                  attributes: ["valor_presupuestado"],
                  required: false,
                },
              ],
            },
    ...
    const data = clientes.map((c) => {
      ...
      const valor_total = procesos.reduce(
        (s, p) => s + (Number(p.propuesta?.valor_presupuestado) || 0),
        0
      );
      return {
        id:             c.id,
  ...
  try {
    ...
    const data = proyectos.map((p) => {
      ...
      const valor_total = procs.reduce(
        (s, pr) => s + (Number(pr.propuesta?.valor_presupuestado) || 0),
        0
      );
      return {
        id:             p.id,
...
export const getProcesosDeProyecto = async (req, res) => {
  try {
    ...
    return res.json({ ok: true, data: procesos });
  } catch (err) {
