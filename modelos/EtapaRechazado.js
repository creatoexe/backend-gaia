import { DataTypes } from "sequelize";
import { sequelize } from "../config/database.js";

export const EtapaRechazado = sequelize.define("EtapaRechazado", {
  id: { type: DataTypes.UUID, defaultValue: DataTypes.UUIDV4, primaryKey: true },
  proceso_id: {
    type: DataTypes.UUID, allowNull: false,
    references: { model: "procesos", key: "id" },
    onDelete: "CASCADE", onUpdate: "CASCADE"
  },
  fecha_rechazo:   { type: DataTypes.DATE,    allowNull: true },
  motivo_categoria: {
    type: DataTypes.ENUM(
      "Precio", "Presupuesto", "Competencia", "Tiempo",
      "Alcance", "Decisión interna", "Sin respuesta", "Otro"
    ),
    allowNull: true
  },
  motivo_detalle:  { type: DataTypes.TEXT,    allowNull: true },
  decision_por:    { type: DataTypes.STRING,  allowNull: true }, // nombre del responsable del cliente
  recuperable: {
    type: DataTypes.ENUM("Sí", "No", "Posiblemente"),
    allowNull: true
  },
  fecha_recontacto: { type: DataTypes.DATE,   allowNull: true }, // si es recuperable
  observaciones:   { type: DataTypes.TEXT,    allowNull: true },
  proximos_pasos:  { type: DataTypes.TEXT,    allowNull: true },
  estado_id: {
    type: DataTypes.UUID, allowNull: true,
    references: { model: "estados", key: "id" }
  },
}, { tableName: "etapa_rechazado", timestamps: false });